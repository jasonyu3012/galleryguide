...
const ArtworkInstance = () => {
  ...
  useEffect(() => {
    ...
    .then(response => {
      ...
      const mapQuery = artworkData.title.replace(' ', "%20")
      ...
      axios.get(`https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&q=${ artworkData.title }&type=video&key=${ googleKey.key }`)
      .then(response => {
